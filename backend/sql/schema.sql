...
CREATE TABLE IF NOT EXISTS furniture_assets (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES users(id) ON DELETE CASCADE,
  generation_job_id UUID UNIQUE REFERENCES generation_jobs(id) ON DELETE CASCADE,
  name TEXT,
  category TEXT,
...
CREATE INDEX IF NOT EXISTS idx_generation_jobs_status_queued ON generation_jobs(status, queued_at);
CREATE INDEX IF NOT EXISTS idx_furniture_assets_user_created ON furniture_assets(user_id, created_at DESC);
